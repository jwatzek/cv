...
\begin{aside}
  ...
  \section{~}
  \section{languages}
    german \& english bilingual $\cdot$ solid french \& spanish $\cdot$ italian, dutch, \& swedish notions
  \section{coding}
    % {\color{red} $\varheartsuit$}
    \faCode{} R $\cdot$ Python $\cdot$ SAS $\cdot$ Matlab $\cdot$ \LaTeX\ $\cdot$ HTML5 $\cdot$ CSS3 $\cdot$ JavaScript 
%  \section{~}
%  \section{orcid}
...
\hangindent=.7cm Calcutt, S.~E., Suchak, M., \textbf{Watzek, J.}, \& de Waal, F. B. M. (\emph{submitted}). Relationship formation and tactical associations in two newly formed groups of captive chimpanzees. Poster to be presented at the Chimpanzees in Context Symposium, Chicago, IL.
