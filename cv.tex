...
\begin{aside}
  ...
  \section{~}
  \section{languages}
    german \& english bilingual $\cdot$ solid french \& spanish $\cdot$ dutch, italian, russian, \& swedish notions
  \section{coding}
    % {\color{red} $\varheartsuit$}
    \faCode{} R $\cdot$ Python $\cdot$ SAS $\cdot$ Matlab $\cdot$ \LaTeX\ $\cdot$ HTML5 $\cdot$ CSS3/Bootstrap $\cdot$ JavaScript/jQuery/D3 
%  \section{~}
%  \section{orcid}
